...
namespace CampChetek.DataAccessLayer
{
    ...
    public class DA
    {
        ...
        public List<GetCalendarEvents> GetCalendarEvents(string start, string end)
        {
            List<GetCalendarEvents> events = new List<GetCalendarEvents>();
            ...
            using (SqlConnection conn = GetConnection())
            {
                ...
                })
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@start", SqlDbType.VarChar).Value = DateTime.Now;
                    }
                    else
                    {
                        cmd.Parameters.AddWithValue("@start", SqlDbType.VarChar).Value = start;
                    }
                    if (String.IsNullOrEmpty(end))
                    {
                        cmd.Parameters.AddWithValue("@end", SqlDbType.VarChar).Value = DateTime.Now;
                    }
                    else
                    {
                        cmd.Parameters.AddWithValue("@end", SqlDbType.VarChar).Value = end;
                    }
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            events.Add(new GetCalendarEvents()
                            {
